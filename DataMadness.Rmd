...
MMdata.blockt.clustered <- data.frame( cbind( MMdata.blockt
                                              , MMdata.blockt.kclusters[[ transformClusterSize - 1 ]]$cluster ) )
colnames( MMdata.blockt.clustered ) <- c( paste0( "block", 1:length( blocks ) ), "cluster" )
ggplot( data.frame( count( MMdata.blockt.clustered$cluster ) )
...
weighted highly: job, political party, favorite sports team
...
# Interesting transition from block6(2) to block13(3), from block16(2) [The 'hippies' (all 7 in block6)]
...
# Plotting only block 6 info [check cluster 2]
plot.clust.distr('block6')

# Interesting transition from block5(5) to block4(5), block12(5) to block4(5), block10(5) to block4(5) [Still undecided]
...
# Plotting only block 5 info [check cluster 5]
plot.clust.distr('block5')

# Interesting transition between block4(1), block12(1) [The 'Believe in Jesus but don't like the word religious'] [about 180 people in both cluster]
create.intersect.df('block4', 'block12', clust_df, norm_first = FALSE)

# Plotting only block 4 info [check cluster 1]
plot.clust.distr('block4')

# Interesting transition from block16(2) and block2(2) [The non religious] [about 275 people in both clusters]
create.intersect.df('block16', 'block2', clust_df, norm_first = FALSE)

# Plotting only block 16 info [check cluster 2]
plot.clust.distr('block16')

# Interesting transition between block16(3) and block12(1) [High integrity and moral values (christians)] [about 270 people]
create.intersect.df('block16', 'block12', clust_df, norm_first = FALSE)

# Plotting only block 12 info [check cluster 6]
plot.clust.distr('block12')

# Clusters in all.blocks.6 => {6: [Non religious], 4: [Hippies/High integrity and moral], 1: [Always undecided], }
