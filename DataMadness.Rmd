...
# Imputing missing values column-wise with empirical distribution
MMdata.imputed <- MMdata_merged
MMdata.imputed[MMdata.imputed == -1] <- NA
MMdata.imputed <- apply(MMdata_merged, 2, function(x){
...
lapply(clusterBlocks, function(x){
  ...
  spm(as.formula(columns), MMdata.imputed, 6000, 3000)
})
```
...
```{r pca_cmp_threshold}
blocks.altweights.raw <- sapply( blocks.pcs, function( pc ) weightsFromPc( 1.05, pc ) )
...
ggplot( blocks.altweights.norm.df[ blocks.altweights.norm.df$L1 != c(15, 17), ]
        , aes( x=name, y=value ) ) + 
  geom_col() + 
  facet_wrap( ~L1, ncol=4, scales="free" ) +
  theme( axis.text.x=element_text( angle=45, hjust=1 ) )
