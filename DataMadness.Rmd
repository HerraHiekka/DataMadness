...
MMdata.blockt.kclusters <- lapply( 1:20, function( k ) kmeans( MMdata.blockt, k, iter.max=100 )  )
...
MMdata.blockt.clustered <- data.frame( cbind( MMdata.blockt, MMdata.blockt.kclusters[[5]]$cluster ) )
colnames( MMdata.blockt.clustered ) <- c( paste0( "block", 1:17 ), "cluster" )
...
MMdata.blockt.clustered.means <- melt( matrix( unlist( lapplyBy( ~cluster
