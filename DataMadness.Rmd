...
for( ib in 1:length( blocks ) )
{
  ...
  print( (off + 1):(off + length( blocks[[ ib ]] ) ) )
  print( blocks.weights.norm[[ ib ]] )
  print( "" )
}

as.matrix( MMdata_merged[] , unlist( blocks ) ] ) %*% blockTransform

lapply( blocks, function( vs ) blocks.weights.norm.df[ blocks.weights.norm.df$name == vs, ] )

MMdata_merged[ 1, vs ] * 

blocks.weights.norm.df[ blocks.weights.norm.df$name == blocks[[1]], ]
